...
VALUES('Yamaha', (SELECT instrument_type_id FROM instrument_type WHERE type_of_instrument = 'Piano'), 250);
...
VALUES('SAL B2', (SELECT skill_level_id FROM skill_level WHERE level = 'intermediate'), (SELECT instrument_type_id FROM instrument_type WHERE type_of_instrument = 'Piano'), (SELECT person_id FROM person WHERE first_name = 'Paris'));
...
VALUES((SELECT person_id FROM person WHERE first_name = 'Iley'), 0, (SELECT school_application_id FROM school_application WHERE first_name = 'Iley'));
...
INSERT INTO persons_skill(skill_level_id, instrument_type_id, person_id)
VALUES((SELECT skill_level_id FROM skill_level WHERE level = 'intermediate'), (SELECT instrument_type_id FROM instrument_type WHERE type_of_instrument = 'Piano'), (SELECT person_id FROM person WHERE first_name = 'Iley'));
...
INSERT INTO instructor_instrument(person_id, instrument_type_id)
VALUES((SELECT person_id FROM person WHERE first_name = 'Paris'), (SELECT instrument_type_id FROM instrument_type WHERE type_of_instrument = 'Piano'));

INSERT INTO lease(end_of_lease_period, renting_fee, terminated, person_id, instrument_id, start_of_lease_period)
VALUES('2023-12-27 14:39:53.662522-05', 150, cast(1 AS bit), (SELECT person_id FROM person WHERE first_name = 'Iley'), (SELECT instrument_id FROM instrument WHERE brand = 'Yamaha'), current_timestamp);

INSERT INTO lease_contract(lease_information_text, person_id, instrument_id, start_of_lease_period)
VALUES('This lease started on 2020-12-27 14:39:53.662522-05 and the lease period ends on 2021-12-27 14:39:53.662522-05, its price is 150 and its for the person with person id 1 and the instrument with instrument id 1', (SELECT person_id FROM person WHERE first_name = 'Iley'), (SELECT instrument_id FROM instrument WHERE brand = 'Yamaha'), (SELECT start_of_lease_period FROM lease WHERE renting_fee = 150));
